function [img,ResX,ResY,ResZ,matrizhist3D]=Reconstruye3D2(directorio,fm,invertida,adaptar)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %                                           %
    ...
    if invertida 
        for p=nlista-1:-fm:0
            ...
            imaux=double(dicomread(filename))/2^NumBits; %lee todos los dicom con los diferentes nombres y los convierte en double
        ...
        for p=fm:fm:nlista-1
            filename=lista(nlista-p).name; %Coge el nombre de cada archivo de la lista
            imaux=double(dicomread(filename))/2^NumBits;
