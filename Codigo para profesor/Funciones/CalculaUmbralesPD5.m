function [umbs,peaks,h1]=CalculaUmbralesPD5(h0,N,tiponormalizacion)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                              %
...
while (length(posm)<nmax & semiancho>=2),
    ...
    for i=1:length(posm0),
        ...
        if (sum(valorm == valor)>0) %Si el valor ya esta dentro se ejecuta lo de abajo
            ...
            valorOr0(j) = 0; %Como ya se guardo para que deje de ser maximo 
        end
...
for i=1:length(posf)-1,
    %
    % Medio: punto central.
    % Mediano: Las areas antes y despues del umbral son proporcionales a los maximos.
    % Minimo: el histograma es minimo.
    %
    inicial = posf(i);
    final = posf(i+1);
    medio = round((inicial+final)/2);
    suma = sum(h1(inicial:final));
    suma0 = suma*h1(inicial)/(h1(inicial)+h1(final));
    sumax = 0;
    index = inicial;
    while(sumax<suma0)
        sumax = sumax + h1(index);
        index=index+1;
    end
    mediano = index;
    [aux1,aux2] = min(h1(inicial:final));
    minimo = inicial+aux2-1;
    aux3 = sort([medio mediano minimo]);
    umbs(i) = aux3(2); 
    peaks=posf;
end